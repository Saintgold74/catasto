...
from app_utils import (
    ...
    LocalitaSelectionDialog, DettagliLegamePossessoreDialog, UserSelectionDialog, # Dialoghi di selezione
    qdate_to_datetime, datetime_to_qdate, # Utility di data
    FPDF_AVAILABLE, gui_esporta_partita_pdf, gui_esporta_partita_json, gui_esporta_partita_csv, # Per EsportazioniWidget
    gui_esporta_possessore_pdf, gui_esporta_possessore_json, gui_esporta_possessore_csv, # Per EsportazioniWidget
    GenericTextReportPDF, # Per ReportisticaWidget
    _hash_password, _verify_password # Se usate in GestioneUtentiWidget direttamente (anche se è in CreateUserDialog)
)
...
try:
    ...
    class DBMError(Exception): pass # ... definizioni fallback come nel file originale
...
class ElencoComuniWidget(QWidget):
    ...
    def load_comuni_data(self):
        ...
        try:
            ...
            if comuni_list:
                ...
                logging.getLogger("CatastoGUI").info("Nessun comune trovato nel database.")
                
        except AttributeError as ae: # Se get_all_comuni_details non esiste
            logging.getLogger("CatastoGUI").error(f"Attributo mancante nel db_manager: {ae}. Assicurati che 'get_all_comuni_details' esista.")
            QMessageBox.critical(self, "Errore Caricamento Dati", f"Funzione dati comuni non trovata: {ae}")
        except Exception as e:
            logging.getLogger("CatastoGUI").error(f"Errore durante il caricamento dei comuni: {e}", exc_info=True)
    ...
    def _get_selected_comune_info(self) -> Optional[Tuple[int, str]]:
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").error(f"Errore in _get_selected_comune_info: {e}", exc_info=True)
    ...
    def mostra_partite_del_comune(self, item: QTableWidgetItem): # Questo è per il doppio click
        ...
        except Exception as e: logging.getLogger("CatastoGUI").error(f"Errore in mostra_partite_del_comune: {e}", exc_info=True); QMessageBox.critical(self, "Errore", f"Errore: {e}")
...
class RicercaPartiteWidget(QWidget):
    ...
    def do_search(self):
        ...
        logging.getLogger("CatastoGUI").debug(f"RicercaPartiteWidget.do_search - Parametri inviati al DBManager:")
        logging.getLogger("CatastoGUI").debug(f"  comune_id: {comune_id} (tipo: {type(comune_id)})")
        logging.getLogger("CatastoGUI").debug(f"  numero_partita: {numero_partita} (tipo: {type(numero_partita)})")
        logging.getLogger("CatastoGUI").debug(f"  possessore: '{possessore}' (tipo: {type(possessore)})")
        logging.getLogger("CatastoGUI").debug(f"  immobile_natura: '{natura}' (tipo: {type(natura)})")
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").debug(f"RicercaPartiteWidget.do_search - Risultati ricevuti dal DBManager (tipo: {type(partite)}):")
            if partite is not None: # Controlla se partite è None prima di len()
                logging.getLogger("CatastoGUI").debug(f"  Numero di partite ricevute: {len(partite)}")
                ...
                logging.getLogger("CatastoGUI").debug("  Nessun risultato (variabile 'partite' è None).")
            # --- Fine Stampa di DEBUG ---
            ...
            if partite: # Verifica se la lista 'partite' non è vuota
                ...
                logging.getLogger("CatastoGUI").info("RicercaPartiteWidget.do_search - Nessuna partita trovata o la lista risultati è vuota.")
                QMessageBox.information(self, "Ricerca Completata", "Nessuna partita trovata con i criteri specificati.")

        except Exception as e:
            logging.getLogger("CatastoGUI").error(f"Errore imprevisto durante RicercaPartiteWidget.do_search: {e}", exc_info=True)
...
class RicercaPossessoriWidget(QWidget):
    ...
    def _perform_search(self):
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").error(f"Errore durante la ricerca possessori (GUI - Consultazione): {e}", exc_info=True)
    ...
    def _seleziona_localita_per_ricerca(self):
        ...
        if dialog.exec_() == QDialog.Accepted: # Se l'utente ha premuto "Seleziona" nel dialogo
            if dialog.selected_localita_id is not None and dialog.selected_localita_name is not None:
                ...
                logging.getLogger("CatastoGUI").info(f"RicercaAvanzataImmobili: Località selezionata ID: {self.selected_localita_id}, Nome: {dialog.selected_localita_name}")
            else:
                # Questo caso è improbabile se _conferma_selezione funziona, ma per sicurezza
                logging.getLogger("CatastoGUI").warning("RicercaAvanzataImmobili: LocalitaSelectionDialog accettato ma nessun ID/nome località valido è stato restituito.")
        ...
        # else: # Dialogo annullato (premuto "Annulla" o chiuso)
            ...
            logging.getLogger("CatastoGUI").info("Selezione località annullata o dialogo chiuso.")
    ...
    def _esegui_ricerca_effettiva(self):
        ...
        try:
            ...
            if immobili_trovati:
             ...
             logging.getLogger("CatastoGUI").error(f"Metodo di ricerca immobili non trovato nel db_manager: {ae}", exc_info=True)
             QMessageBox.critical(self, "Errore Interno", f"Funzionalità di ricerca non implementata correttamente nel gestore DB: {ae}")
        except Exception as e:
            logging.getLogger("CatastoGUI").error(f"Errore durante la ricerca avanzata immobili: {e}", exc_info=True)
...
class InserimentoComuneWidget(QWidget):
    def __init__(self, parent: Optional[QWidget] = None, # parent come primo argomento dopo self
        ...
        if db_manager is None:
            logging.getLogger("CatastoGUI").critical("InserimentoComuneWidget: db_manager non fornito all'inizializzazione!")
    ...
    def _apri_dettaglio_modifica_periodo(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            logging.getLogger("CatastoGUI").info(f"Dialogo dettagli/modifica periodo ID {selected_periodo_id} chiuso con successo. Aggiorno elenco periodi.")
            self._carica_elenco_periodi()
        else:
            logging.getLogger("CatastoGUI").info(f"Dialogo dettagli/modifica periodo ID {selected_periodo_id} annullato o chiuso.")
    ...
    def _carica_elenco_periodi(self):
        ...
        try:
            logging.getLogger("CatastoGUI").info("Chiamata a db_manager.get_historical_periods()...") 
            periodi = self.db_manager.get_historical_periods()
            logging.getLogger("CatastoGUI").info(f"Elenco periodi ricevuto da DBManager (tipo: {type(periodi)}): {periodi if periodi is not None else 'None'}")
            if periodi: 
                logging.getLogger("CatastoGUI").info(f"Numero di periodi ricevuti: {len(periodi)}")
                ...
                logging.getLogger("CatastoGUI").warning("Nessun periodo storico restituito da db_manager.get_historical_periods() o la lista è vuota.")
            ...
            logging.getLogger("CatastoGUI").error(f"Errore imprevisto durante _carica_elenco_periodi: {e}", exc_info=True)
    ...
    def inserisci_comune(self):
        ...
        logging.getLogger("CatastoGUI").debug(f"InserimentoComuneWidget: Invio al DBManager -> nome='{nome_comune}', prov='{provincia}', regione='{regione}', periodo_id='{periodo_id_val}', utente='{username_per_log}'")
        try:
            ...
            logging.getLogger("CatastoGUI").warning(f"Unicità violata inserendo comune '{nome_comune}': {str(uve)}")
            QMessageBox.critical(self, "Errore di Unicità", str(uve))
        except DBDataError as dde:
            logging.getLogger("CatastoGUI").warning(f"Dati non validi per comune '{nome_comune}': {str(dde)}")
            QMessageBox.warning(self, "Dati Non Validi", str(dde))
        except DBMError as dbe: 
            logging.getLogger("CatastoGUI").error(f"Errore DB inserendo comune '{nome_comune}': {str(dbe)}", exc_info=True)
            QMessageBox.critical(self, "Errore Database", str(dbe))
        except Exception as e:
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto inserendo comune '{nome_comune}': {e}", exc_info=True)
...
class InserimentoPossessoreWidget(QWidget):
    ...
    def _load_comuni_for_combo(self):
        try:
            ...
            logging.getLogger("CatastoGUI").error(f"Errore caricamento comuni in InserimentoPossessoreWidget: {e}")
    ...
    def _salva_possessore(self):
        ...
        if comune_id_selezionato_data is not None:
            try:
                ...
                logging.getLogger("CatastoGUI").error(f"ID comune non valido nella combobox: {comune_id_selezionato_data}")
        ...
        try:
            ...
            if new_possessore_id is not None: # Controllo aggiuntivo, anche se dovrebbe sempre esserci un ID o un'eccezione.
                ...
                logging.getLogger("CatastoGUI").error("create_possessore ha restituito None senza sollevare un'eccezione esplicita.")
            ...
            logging.getLogger("CatastoGUI").warning(f"Errore di unicità durante il salvataggio del possessore '{nome_completo}': {uve.message}")
            ...
            logging.getLogger("CatastoGUI").warning(f"Errore nei dati forniti per il possessore '{nome_completo}': {dde.message}")
            ...
            logging.getLogger("CatastoGUI").error(f"Errore database gestito durante il salvataggio del possessore '{nome_completo}': {dbe.message}", exc_info=True)
            ...
            logging.getLogger("CatastoGUI").critical(f"Errore critico e imprevisto durante il salvataggio del possessore '{nome_completo}': {e}", exc_info=True)
...
class InserimentoLocalitaWidget(QWidget):
    ...
    def refresh_localita(self):
        ...
        if self.comune_id: # Assicurati che comune_id sia stato impostato da select_comune
            logging.getLogger("CatastoGUI").info(f"DEBUG: InserimentoLocalitaWidget - Chiamata refresh_localita per comune ID: {self.comune_id}")
            try:
                ...
                if localita_list:
                    ...
                    logging.getLogger("CatastoGUI").info(f"Nessuna località trovata per il comune ID: {self.comune_id} in InserimentoLocalitaWidget.")
                ...
                logging.getLogger("CatastoGUI").error(f"Metodo get_localita_by_comune non trovato nel db_manager (chiamato da InserimentoLocalitaWidget): {ae}")
                QMessageBox.critical(self, "Errore Funzionalità", "Funzione per caricare località non implementata correttamente.")
            except Exception as e:
                logging.getLogger("CatastoGUI").error(f"Errore durante l'aggiornamento della lista località per comune {self.comune_id}: {e}", exc_info=True)
                QMessageBox.critical(self, "Errore Caricamento", f"Impossibile aggiornare la lista delle località: {e}")
        else:
            logging.getLogger("CatastoGUI").info("InserimentoLocalitaWidget - refresh_localita: Nessun comune_id selezionato.")
...
class RegistrazioneProprietaWidget(QWidget):
    ...
    def _salva_proprieta(self):
        logging.getLogger("CatastoGUI").info("Avvio registrazione nuova proprietà...")
        if not self.comune_id:
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").error(f"Errore serializzazione JSON per nuova proprietà: {te}")
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").error(f"Errore DB registrazione proprietà: {e_db}")
            QMessageBox.critical(self, "Errore Database", str(e_db))
        except Exception as e_gen:
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto registrazione proprietà: {e_gen}", exc_info=True)
    ...
    def _pulisci_form_registrazione(self):
        ...
        logging.getLogger("CatastoGUI").info("Pulizia campi del form Registrazione Proprietà.")
        ...
        logging.getLogger("CatastoGUI").info("Campi form Registrazione Proprietà puliti.")
class OperazioniPartitaWidget(QWidget):
    ...
    def _esegui_duplicazione_partita(self):
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto duplicazione: {e_gen}", exc_info=True)
    ...
    def _esegui_trasferimento_immobile(self):
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto trasferimento immobile: {e_gen}", exc_info=True)
            QMessageBox.critical(self, "Errore Imprevisto", f"Errore:\n{type(e_gen).__name__}: {str(e_gen)}")
    ...
    def _pp_carica_immobili_per_selezione(self, immobili_data: List[Dict[str, Any]]):
        if not hasattr(self, 'pp_immobili_da_selezionare_table'):
            logging.getLogger("CatastoGUI").error("Tabella 'pp_immobili_da_selezionare_table' non inizializzata.")
            return
    ...
    def _pp_aggiungi_nuovo_possessore(self):
        ...
        if dialog_sel_poss.exec_() == QDialog.Accepted:
            ...
            else: logging.getLogger("CatastoGUI").warning("PossessoreSelectionDialog non ha restituito 'selected_possessore'."); return
        else: logging.getLogger("CatastoGUI").info("Aggiunta possessore per PP annullata (selezione/creazione)."); return
        ...
        else: logging.getLogger("CatastoGUI").info("Aggiunta dettagli legame per PP annullata.")
    ...
    def _esegui_passaggio_proprieta(self):
        logging.getLogger("CatastoGUI").info("Avvio _esegui_passaggio_proprieta.")
        ...
        logging.getLogger("CatastoGUI").debug(f"PP: Lista possessori inviata al DBManager: {lista_possessori_per_db}")
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").error(f"Errore durante la registrazione del passaggio di proprietà: {str(e)}", exc_info=True)
            QMessageBox.critical(self, "Errore Operazione", f"Impossibile registrare il passaggio di proprietà:\n{str(e)}")
        except Exception as e_gen:
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto passaggio proprietà: {e_gen}", exc_info=True)
    ...
    def seleziona_e_carica_partita_sorgente(self, partita_id: int):
        ...
        logging.getLogger("CatastoGUI").info(f"OperazioniPartitaWidget: Impostazione partita sorgente ID: {partita_id} da chiamata esterna.")
...
class EsportazioniWidget(QWidget):
    ...
    def _cerca_possessore_per_export(self):
        logging.getLogger("CatastoGUI").info(">>> _cerca_possessore_per_export: Metodo chiamato.")
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if hasattr(dialog, 'selected_possessore') and \
               ...
               dialog.selected_possessore.get('id') is not None:
                ...
                logging.getLogger("CatastoGUI").info(f"_cerca_possessore_per_export: Possessore selezionato ID {self.selected_possessore_id_export}")
                ...
                logging.getLogger("CatastoGUI").warning("_cerca_possessore_per_export: Dialogo accettato ma 'selected_possessore' o il suo 'id' non validi.")
            # --- FINE MODIFICA ---
            ...
            logging.getLogger("CatastoGUI").info("_cerca_possessore_per_export: Dialogo selezione annullato.")
...
class ReportisticaWidget(QWidget):
    def __init__(self, db_manager, parent=None):
        ...
        self.search_partita_button = QPushButton("Cerca..."); self.search_partita_button.clicked.connect(self.search_partita)
        input_layout.addWidget(partita_id_label); input_layout.addWidget(self.partita_id_edit); input_layout.addWidget(self.search_partita_button)
        certificato_layout.addLayout(input_layout)

        self.generate_cert_button = QPushButton("Genera Certificato"); self.generate_cert_button.clicked.connect(self.generate_certificato)
        certificato_layout.addWidget(self.generate_cert_button)
        self.certificato_text = QTextEdit(); self.certificato_text.setReadOnly(True); self.certificato_text.setFontFamily("Courier New")
        certificato_layout.addWidget(self.certificato_text)
        
        export_buttons_cert_layout = QHBoxLayout() # Layout per i pulsanti di esportazione
        self.export_cert_txt_button = QPushButton("Esporta in TXT"); self.export_cert_txt_button.clicked.connect(lambda: self.export_report(self.certificato_text.toPlainText(), "certificato_proprieta", "txt"))
        self.export_cert_pdf_button = QPushButton("Esporta in PDF"); self.export_cert_pdf_button.clicked.connect(self._export_certificato_pdf) # NUOVO METODO
        self.export_cert_pdf_button.setEnabled(FPDF_AVAILABLE)
        export_buttons_cert_layout.addWidget(self.export_cert_txt_button)
        export_buttons_cert_layout.addWidget(self.export_cert_pdf_button) # AGGIUNTO PULSANTE PDF
        ...
        input_gen_layout.addWidget(partita_id_gen_label); input_gen_layout.addWidget(self.partita_id_gen_edit); input_gen_layout.addWidget(self.search_partita_gen_button)
        ...
        self.genealogico_text = QTextEdit(); self.genealogico_text.setReadOnly(True); self.genealogico_text.setFontFamily("Courier New")
        genealogico_layout.addWidget(self.genealogico_text)
        
        export_buttons_gen_layout = QHBoxLayout()
        self.export_gen_txt_button = QPushButton("Esporta in TXT"); self.export_gen_txt_button.clicked.connect(lambda: self.export_report(self.genealogico_text.toPlainText(), "report_genealogico", "txt"))
        self.export_gen_pdf_button = QPushButton("Esporta in PDF"); self.export_gen_pdf_button.clicked.connect(self._export_genealogico_pdf) # NUOVO METODO
        self.export_gen_pdf_button.setEnabled(FPDF_AVAILABLE)
        export_buttons_gen_layout.addWidget(self.export_gen_txt_button)
        export_buttons_gen_layout.addWidget(self.export_gen_pdf_button) # AGGIUNTO PULSANTE PDF
        ...
        input_pos_layout.addWidget(possessore_id_label); input_pos_layout.addWidget(self.possessore_id_edit); input_pos_layout.addWidget(self.search_possessore_button)
        ...
        self.possessore_text = QTextEdit(); self.possessore_text.setReadOnly(True); self.possessore_text.setFontFamily("Courier New")
        possessore_layout.addWidget(self.possessore_text)
        
        export_buttons_pos_layout = QHBoxLayout()
        self.export_pos_txt_button = QPushButton("Esporta in TXT"); self.export_pos_txt_button.clicked.connect(lambda: self.export_report(self.possessore_text.toPlainText(), "report_possessore", "txt"))
        self.export_pos_pdf_button = QPushButton("Esporta in PDF"); self.export_pos_pdf_button.clicked.connect(self._export_possessore_pdf) # NUOVO METODO
        self.export_pos_pdf_button.setEnabled(FPDF_AVAILABLE)
        export_buttons_pos_layout.addWidget(self.export_pos_txt_button)
        export_buttons_pos_layout.addWidget(self.export_pos_pdf_button) # AGGIUNTO PULSANTE PDF
        ...
        data_inizio_label = QLabel("Data inizio:"); self.data_inizio_edit = QDateEdit(); self.data_inizio_edit.setCalendarPopup(True); self.data_inizio_edit.setDate(QDate.currentDate().addYears(-1)); self.data_inizio_check = QCheckBox("Usa filtro"); self.data_inizio_check.setChecked(True)
        data_fine_label = QLabel("Data fine:"); self.data_fine_edit = QDateEdit(); self.data_fine_edit.setCalendarPopup(True); self.data_fine_edit.setDate(QDate.currentDate()); self.data_fine_check = QCheckBox("Usa filtro"); self.data_fine_check.setChecked(True)
        richiedente_label = QLabel("Richiedente:"); self.richiedente_edit = QLineEdit(); self.richiedente_edit.setPlaceholderText("Qualsiasi richiedente")
        filters_layout.addWidget(data_inizio_label, 0, 0); filters_layout.addWidget(self.data_inizio_edit, 0, 1); filters_layout.addWidget(self.data_inizio_check, 0, 2)
        filters_layout.addWidget(data_fine_label, 1, 0); filters_layout.addWidget(self.data_fine_edit, 1, 1); filters_layout.addWidget(self.data_fine_check, 1, 2)
        filters_layout.addWidget(richiedente_label, 2, 0); filters_layout.addWidget(self.richiedente_edit, 2, 1, 1, 2)
        consultazioni_layout.addLayout(filters_layout)
        self.generate_cons_button = QPushButton("Genera Report Consultazioni"); self.generate_cons_button.clicked.connect(self.generate_consultazioni)
        consultazioni_layout.addWidget(self.generate_cons_button)
        self.consultazioni_text = QTextEdit(); self.consultazioni_text.setReadOnly(True); self.consultazioni_text.setFontFamily("Courier New")
        consultazioni_layout.addWidget(self.consultazioni_text)
        
        export_buttons_cons_layout = QHBoxLayout()
        self.export_cons_txt_button = QPushButton("Esporta in TXT"); self.export_cons_txt_button.clicked.connect(lambda: self.export_report(self.consultazioni_text.toPlainText(), "report_consultazioni", "txt"))
        self.export_cons_pdf_button = QPushButton("Esporta in PDF"); self.export_cons_pdf_button.clicked.connect(self._export_consultazioni_pdf) # NUOVO METODO
        self.export_cons_pdf_button.setEnabled(FPDF_AVAILABLE)
        export_buttons_cons_layout.addWidget(self.export_cons_txt_button)
        export_buttons_cons_layout.addWidget(self.export_cons_pdf_button) # AGGIUNTO PULSANTE PDF
    ...
    def search_possessore(self):
        ...
        logging.getLogger("CatastoGUI").info("ReportisticaWidget: Apertura dialogo ricerca possessore.")
        ...
        dialog.setWindowTitle("Seleziona Possessore per Report") # Titolo specifico
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if hasattr(dialog, 'selected_possessore') and \
               ...
               dialog.selected_possessore.get('id') is not None:
                ...
                self.possessore_id_edit.setValue(selected_id) # Aggiorna lo QSpinBox
                logging.getLogger("CatastoGUI").info(f"ReportisticaWidget: Possessore selezionato ID: {selected_id}")
                # Opzionale: potresti voler mostrare il nome del possessore vicino allo spinbox
            else:
                logging.getLogger("CatastoGUI").warning("ReportisticaWidget: Dialogo selezione possessore accettato ma nessun possessore valido selezionato.")
        else:
            logging.getLogger("CatastoGUI").info("ReportisticaWidget: Selezione possessore annullata.")
    ...
    def export_report(self, text_content: str, report_type_name: str, file_extension: str): # Modificato per estensione
        ...
        if filename:
            try:
                ...
                QMessageBox.information(self, "Esportazione Completata", f"Report salvato con successo in:\n{filename}")
            except Exception as e:
                logging.getLogger("CatastoGUI").error(f"Errore durante il salvataggio del file {file_extension.upper()} per report {report_type_name}: {e}")
    ...
    def _export_generic_text_to_pdf(self, text_content: str, default_filename_prefix: str, pdf_report_title: str):
        ...
        if not FPDF_AVAILABLE:
            ...
            return
        if not text_content:
            QMessageBox.warning(self, "Nessun Contenuto", f"Nessun testo da esportare in PDF per '{pdf_report_title}'.")
            return

        default_filename = f"{default_filename_prefix}_{date.today().isoformat()}.pdf"
        filename_pdf, _ = QFileDialog.getSaveFileName(self, f"Salva PDF - {pdf_report_title}", default_filename, "File PDF (*.pdf)")

        if filename_pdf:
            try:
                pdf = GenericTextReportPDF(report_title=pdf_report_title) # Usa la nuova classe PDF generica
                pdf.add_page()
                pdf.add_report_text(text_content)
                pdf.output(filename_pdf)
                QMessageBox.information(self, "Esportazione PDF Completata", f"Report PDF salvato con successo in:\n{filename_pdf}")
            except Exception as e:
                logging.getLogger("CatastoGUI").error(f"Errore durante la generazione del PDF per '{pdf_report_title}': {e}", exc_info=True)
...
class StatisticheWidget(QWidget):
    ...
    def refresh_stats_comune(self):
        ...
        if not self.db_manager or not self.db_manager.pool: # <-- CONTROLLO AGGIUNTO
            logging.getLogger("CatastoGUI").warning("StatisticheWidget: Pool DB non inizializzato. Impossibile caricare statistiche comuni.")
            # Opzionale: mostra messaggio nella tabella
            self.stats_comune_table.setRowCount(1)
            item_msg = QTableWidgetItem("Database non pronto per caricare le statistiche.")
            item_msg.setTextAlignment(Qt.AlignCenter)
            self.stats_comune_table.setItem(0,0, item_msg)
            self.stats_comune_table.setSpan(0,0,1, self.stats_comune_table.columnCount())
            return
    ...
    def refresh_immobili_tipologia(self):
        ...
        if not self.db_manager or not self.db_manager.pool: # <-- CONTROLLO AGGIUNTO
            logging.getLogger("CatastoGUI").warning("StatisticheWidget: Pool DB non inizializzato. Impossibile caricare statistiche immobili.")
            self.immobili_table.setRowCount(1)
            item_msg = QTableWidgetItem("Database non pronto per caricare le statistiche immobili.")
            item_msg.setTextAlignment(Qt.AlignCenter)
    ...
    def _esegui_verifica_integrita(self):
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").error("Errore imprevisto in _esegui_verifica_integrita", exc_info=True)
...
class AuditLogViewerWidget(QWidget):
    ...
    def _fetch_and_display_logs(self, page_number: int = 1):
        ...
        if not self.db_manager or not self.db_manager.pool:
            logging.getLogger("CatastoGUI").warning("AuditLogViewer: _fetch_and_display_logs chiamato ma il pool non è inizializzato. Interruzione.")
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").error(f"AuditLogViewer: Errore DBManager in _fetch_and_display_logs: {str(e)}", exc_info=False)
            if hasattr(self, 'log_table'):
                self.log_table.setRowCount(1)
                item_msg = QTableWidgetItem(f"Errore caricamento log: {str(e)}")
                item_msg.setTextAlignment(Qt.AlignCenter)
                self.log_table.setItem(0,0, item_msg)
                self.log_table.setSpan(0,0,1, self.log_table.columnCount())
        except Exception as e:
            logging.getLogger("CatastoGUI").error(f"AuditLogViewer: Errore generico in _fetch_and_display_logs: {e}", exc_info=True)
    ...
    def _load_initial_data(self):
        if not self.db_manager or not self.db_manager.pool: # Questo controllo è cruciale
            logging.getLogger("CatastoGUI").warning("AuditLogViewer: Database principale non configurato o pool non inizializzato. Log non caricati.")
            if hasattr(self, 'log_table'):
                self.log_table.setRowCount(1)
                item_msg = QTableWidgetItem("Database non pronto per caricare i log di audit.")
                item_msg.setTextAlignment(Qt.AlignCenter)
                self.log_table.setItem(0,0, item_msg)
                self.log_table.setSpan(0,0,1, self.log_table.columnCount())
        ...
        logging.getLogger("CatastoGUI").info("AuditLogViewer: Pool inizializzato, procedo con il caricamento dei log iniziali.")
...
class AdminDBOperationsWidget(QWidget):
    def __init__(self, db_manager: 'CatastoDBManager', parent=None):
        ...
        self.script_base_path = os.path.join(os.path.dirname(__file__), "sql_scripts")
    ...
    def _initUI(self):
        ...
        create_db_group = QGroupBox("0. Creazione Database Principale ('catasto_storico')")
        create_db_layout = QVBoxLayout(create_db_group)
        self.btn_run_create_database = QPushButton(QApplication.style().standardIcon(QStyle.SP_CommandLink), " Crea Database 'catasto_storico'")
        self.btn_run_create_database.setToolTip("Esegue lo script '01_creazione-database.sql' per creare il database principale.\nRichiede credenziali utente PostgreSQL con privilegi di creazione database.")
        self.btn_run_create_database.clicked.connect(self._run_script_crea_database)
        create_db_layout.addWidget(self.btn_run_create_database)
        create_db_layout.addWidget(QLabel("<i>Eseguire solo se il database 'catasto_storico' non esiste.</i>"))
        main_layout.addWidget(create_db_group)

        # --- 1. Sezione Configurazione Iniziale ---
        config_group = QGroupBox("1. Setup Struttura Database (Tabelle, Funzioni, ecc.)")
        config_layout = QVBoxLayout(config_group)
        self.btn_run_initial_setup = QPushButton(QApplication.style().standardIcon(QStyle.SP_MediaPlay), " Esegui Script Configurazione Struttura")
        self.btn_run_initial_setup.setToolTip("Esegue script SQL per creare tabelle, viste, funzioni, ecc. (dopo creazione DB)")
        self.btn_run_initial_setup.clicked.connect(self._run_all_config_scripts)
        config_layout.addWidget(self.btn_run_initial_setup)
        config_layout.addWidget(QLabel("<i>Eseguire dopo che il database 'catasto_storico' è stato creato e il pool è attivo.</i>"))
        main_layout.addWidget(config_group)

        # --- 2. Sezione Carico Dati SQL ---
        load_sql_group = QGroupBox("2. Caricamento Dati di Esempio (Script SQL)")
        load_sql_layout = QVBoxLayout(load_sql_group)
        self.btn_run_sql_data_load = QPushButton(QApplication.style().standardIcon(QStyle.SP_ArrowUp), " Esegui Script Caricamento Dati SQL")
        self.btn_run_sql_data_load.setToolTip("Esegue lo script SQL ('04_dati_stress_test.sql') per popolare il DB.")
        self.btn_run_sql_data_load.clicked.connect(self._run_sql_data_load_script)
        ...
        self.btn_run_python_populate = QPushButton(QApplication.style().standardIcon(QStyle.SP_ArrowRight), " Esegui Popolamento Dati Python")
        self.btn_run_python_populate.setToolTip("Genera e inserisce dati di esempio usando la logica Python dell'applicazione.")
        self.btn_run_python_populate.clicked.connect(self._run_python_data_population)
        ...
        self.btn_run_db_wipe = QPushButton(QApplication.style().standardIcon(QStyle.SP_TrashIcon), " Esegui Script Cancellazione Database")
        self.btn_run_db_wipe.setStyleSheet("QPushButton { background-color: #d9534f; border-color: #d43f3a; color: white; } QPushButton:hover { background-color: #c9302c; } QPushButton:pressed { background-color: #ac2925; }")
        self.btn_run_db_wipe.setToolTip("ATTENZIONE: Esegue lo script 'drop_db.sql' per cancellare il database specificato.")
        self.btn_run_db_wipe.clicked.connect(self._run_db_deletion_script)
        delete_db_layout.addWidget(self.btn_run_db_wipe)
        delete_db_layout.addWidget(QLabel("<font color='red'><b>ATTENZIONE MASSIMA: Operazioni altamente distruttive. Richiederanno multiple conferme.</b></font>"))
    ...
    def _log_output(self, message: str, error: bool = False):
        ...
        if error:
            self.output_log.append(f"<font color='red'>{formatted_message}</font>")
        else:
            self.output_log.append(formatted_message)
        QApplication.processEvents() # Per forzare l'aggiornamento della UI durante operazioni lunghe
    ...
    def _run_script_crea_database(self):
        ...
        script_name = "01_creazione-database.sql" # Lo script che contiene "CREATE DATABASE catasto_storico;"
        script_path = os.path.join(self.script_base_path, script_name)

        if not os.path.exists(script_path):
            QMessageBox.critical(self, "Script Mancante", f"Lo script di creazione database '{script_name}' non è stato trovato in:\n{script_path}")
            self._log_output(f"ERRORE: Script '{script_name}' non trovato.", error=True)
            return

        target_dbname = self.db_manager.get_current_dbname() # Dovrebbe essere "catasto_storico"
        if not target_dbname:
            QMessageBox.critical(self, "Errore Configurazione", "Nome del database target non definito nel DBManager."); return
        ...
        if reply == QMessageBox.No:
            self._log_output("Creazione database annullata dall'utente."); return
        
        self._disable_all_buttons(True) # Disabilita pulsanti durante l'operazione
        ...
        self._log_output("Tentativo di inizializzazione del pool per il database principale...")
        QApplication.processEvents()
        if self.db_manager.initialize_main_pool():
            self._log_output("Pool per il database principale ('{target_dbname}') inizializzato con successo!", error=False)
            QMessageBox.information(self, "Successo", f"Database '{target_dbname}' creato (o già esistente) e pool inizializzato.\nPuoi procedere con la configurazione delle tabelle e delle funzioni.")
            ...
            if isinstance(main_window, QMainWindow) and hasattr(main_window, 'db_status_label'):
                 main_window.db_status_label.setText(f"Database: Connesso ({target_dbname})")
            self._update_admin_buttons_state() # Aggiorna stato pulsanti
        else:
            self._log_output(f"FALLITO inizializzazione pool per il database '{target_dbname}' dopo tentativo di creazione.", error=True)
            QMessageBox.critical(self, "Errore Pool", f"Database '{target_dbname}' potrebbe essere stato creato, ma è fallita l'inizializzazione del pool di connessione. Controllare i log.")
            self._update_admin_buttons_state() # Aggiorna stato pulsanti
    ...
    def _run_all_config_scripts(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            self._execute_sql_script_list_via_psql(config_scripts, "Configurazione Struttura Database")

    def _run_sql_data_load_script(self):
        if not self.db_manager.pool:
            QMessageBox.warning(self, "Database Non Pronto", "Il pool principale non è attivo. Eseguire prima la creazione/configurazione del DB."); return
        ...
        reply = QMessageBox.question(self, "Conferma Caricamento Dati SQL",
                                     ...
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            self._execute_sql_script_list_via_psql(data_load_scripts, "Caricamento Dati da SQL")
    def _execute_sql_script_list_via_psql(self, script_names: List[str], operation_name: str, dbname_override: Optional[str] = None):
        ...
        """
        self._disable_all_buttons(True) # Disabilita i pulsanti durante l'intera sequenza
        self.output_log.clear()
        self._log_output(f"Avvio operazione: {operation_name}...")
        
        if not self.db_manager: # Non dovrebbe accadere se la UI è ben gestita
            self._log_output("ERRORE CRITICO: DB Manager non disponibile.", error=True)
        ...
        db_user_main = conn_params.get("user", "postgres") # Utente del DB principale
        ...
        if not target_db_for_psql:
            QMessageBox.critical(self, "Errore Configurazione", f"Nome del database di destinazione per {operation_name} non specificato o non recuperabile.")
            self._log_output(f"ERRORE: Nome database non specificato per {operation_name}.", error=True)
            self._disable_all_buttons(False)
            self._update_admin_buttons_state()
            return
        ...
        if dbname_override and dbname_override.lower() == "postgres" and db_user_main.lower() != "postgres":
            ...
            # Per ora, usiamo l'utente principale, assumendo abbia i privilegi necessari sul DB di manutenzione.
             ...
             if risposta == QMessageBox.No:
                 self._log_output(f"{operation_name} annullato: utente '{db_user_main}' potrebbe non avere privilegi sufficienti.", error=True)
                 self._disable_all_buttons(False); self._update_admin_buttons_state(); return
        ...
        password, ok = QInputDialog.getText(self, f"Autenticazione DB per {operation_name}",
                                            ...
                                            QLineEdit.Password)
        if not ok:
            self._log_output(f"{operation_name} annullato: dialogo password chiuso."); 
            self._disable_all_buttons(False); self._update_admin_buttons_state(); return
        ...
        if not password.strip() and operation_name not in ["Cancellazione Database"]: # La cancellazione su 'postgres' potrebbe non aver bisogno di password utente
             QMessageBox.warning(self, "Password Mancante", "La password non può essere vuota per questa operazione.")
             self._log_output(f"{operation_name} fallito: password non fornita.", error=True)
             self._disable_all_buttons(False); self._update_admin_buttons_state(); return

        psql_exe = self.db_manager._resolve_executable_path(user_provided_path="", default_name="psql.exe")
        if not psql_exe:
            QMessageBox.critical(self, "Errore Prerequisito", "L'eseguibile 'psql.exe' non è stato trovato. Specificare il percorso nelle impostazioni dell'applicazione (se disponibili) o assicurarsi che sia nel PATH di sistema.")
        ...
        for script_name in script_names:
            script_path = os.path.join(self.script_base_path, script_name)
            if not os.path.exists(script_path):
                self._log_output(f"ERRORE: Script '{script_name}' non trovato in {script_path}", error=True)
                all_scripts_successful = False; break
            ...
            args = ["-U", utente_per_connessione_psql, "-h", db_host, "-p", db_port, "-d", target_db_for_psql, "-f", script_path, "-v", "ON_ERROR_STOP=1"]
            ...
            self._log_output(f"Esecuzione script: '{script_name}' sul database '{target_db_for_psql}'...")
            self._log_output(f"Comando: {psql_exe} {' '.join(args)}") # Mostra il comando
            QApplication.processEvents()

            self.current_process.start(psql_exe, args)
            
            if not self.current_process.waitForFinished(-1): # Attendi indefinitamente
            ...
            if exit_status == QProcess.CrashExit or exit_code != 0:
                ...
                self._log_output(f"SUCCESSO (comando terminato): Script '{script_name}' completato con codice 0.")
            
            self.current_process = None # Rilascia riferimento per il prossimo script

        self._handle_script_finished_summary(all_scripts_successful, operation_name)
    ...
    def _handle_script_finished_summary(self, all_successful: bool, operation_name: str):
        ...
        self._disable_all_buttons(False)
        self._update_admin_buttons_state() # Aggiorna lo stato dei pulsanti in base al pool
    ...
    def _run_python_data_population(self):
        ...
        self.output_log.clear()
        self._log_output("Avvio popolamento dati di esempio guidato da Python...")
        QApplication.setOverrideCursor(Qt.WaitCursor)
        self._disable_all_buttons(True)
        
        try:
            num_comuni_creati = 0
            num_comuni_da_creare, ok = QInputDialog.getInt(self, "Popolamento Python", "Quanti comuni di esempio vuoi creare?", 2, 1, 10, 1)
            if not ok:
                ...
                for i in range(num_comuni_da_creare):
                    ...
                    utente_attuale_per_log = "admin_setup" # O prendilo da self.main_window.logged_in_user_info se disponibile
                    ...
                    try:
                        ...
                        if comune_id:
                            self._log_output(f"Creato Comune: '{nome_comune_esempio}' (ID: {comune_id})")
                            num_comuni_creati += 1
                        ...
                        self._log_output(f"Fallita creazione Comune '{nome_comune_esempio}': {e_comune}", error=True)
                    QApplication.processEvents()
                
                message = f"Popolamento Python completato.\nComuni creati: {num_comuni_creati}"
                self._log_output(message)
                QMessageBox.information(self, "Popolamento Completato", message)

        except Exception as e: # Cattura qualsiasi altra eccezione
            self._log_output(f"ERRORE imprevisto durante il popolamento Python: {e}", error=True)
            logging.getLogger("CatastoGUI").critical("Errore imprevisto popolamento Python", exc_info=True)
            QMessageBox.critical(self, "Errore Popolamento Imprevisto", f"Errore: {e}")
        finally:
            self._disable_all_buttons(False)
            QApplication.restoreOverrideCursor()
            self._update_admin_buttons_state() # Ri-abilita i pulsanti in base allo stato del pool
    ...
    def _run_db_deletion_script(self):
        ...
        script_path = os.path.join(self.script_base_path, script_name)
        if not os.path.exists(script_path):
            QMessageBox.critical(self, "Script Mancante", f"Script '{script_name}' non trovato."); return

        dbname_corrente = self.db_manager.get_current_dbname()
        if not dbname_corrente:
             QMessageBox.critical(self, "Errore", "Nome DB target non recuperabile."); return
        ...
        reply1 = QMessageBox.critical(self, "ATTENZIONE MASSIMA - CANCELLAZIONE!",
                                      ...
                                      "Procedere?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply1 == QMessageBox.No: self._log_output("Cancellazione DB annullata."); return
        ...
        if not ok: self._log_output("Cancellazione DB annullata."); return
        if testo_conferma.strip() != dbname_corrente:
            QMessageBox.critical(self, "Conferma Fallita", "Nome DB non corrispondente. Annullato.");
            self._log_output("Cancellazione DB annullata: nome conferma errato.", error=True); return
        ...
        if self.db_manager.pool:
            self._log_output(f"Chiusura del pool di connessioni per '{dbname_corrente}' prima della cancellazione...")
        ...
        self._log_output(f"ATTENZIONE: Database '{dbname_corrente}' dovrebbe essere stato cancellato. L'applicazione potrebbe necessitare di riavvio.", error=True)
        self._update_admin_buttons_state() # Aggiorna stato pulsanti (Crea DB dovrebbe essere abilitato ora)
        # Aggiorna status bar nella finestra principale per riflettere che il DB non esiste più
        main_window = self.window().window() # Trova la CatastoMainWindow
        if isinstance(main_window, QMainWindow) and hasattr(main_window, 'db_status_label'):
                main_window.db_status_label.setText(f"Database: NON ESISTENTE ({dbname_corrente})")
        if isinstance(main_window, QMainWindow) and hasattr(main_window, 'update_ui_based_on_role'):
                ...
                main_window.logged_in_user_info = {'ruolo': 'admin_offline'} # Forza la modalità limitata
                main_window.update_ui_based_on_role()

    def _handle_script_finished(self, exit_code, exit_status, operation_name: str):
        self.output_log.append(f"<hr>DEBUG Processo '{operation_name}' terminato. ExitCode: {exit_code}, ExitStatus: {exit_status}<hr>")
        success_message_box_needed = False
        if exit_status == QProcess.CrashExit:
            msg = f"ERRORE CRITICO: Processo {operation_name} terminato inaspettatamente (crash)."
            self._log_output(msg, error=True); QMessageBox.critical(self, "Errore Processo", msg)
        elif exit_code != 0:
            msg = f"FALLITO: Processo {operation_name} terminato con codice d'errore: {exit_code}. Controllare output."
            self._log_output(msg, error=True); QMessageBox.warning(self, "Operazione Fallita", msg)
        else:
            ...
            self._log_output(msg); success_message_box_needed = True
...
class RegistraConsultazioneWidget(QWidget):
    ...
    def _salva_consultazione(self):
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").error(f"Errore durante la registrazione della consultazione: {str(e)}", exc_info=False)
            QMessageBox.critical(self, "Errore Registrazione", str(e))
        except Exception as e_gen:
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto registrazione consultazione: {e_gen}", exc_info=True)
    ...
    def export_to_json(self):
        ...
        if file_path:
            try:
                ...
                logging.getLogger("CatastoGUI").error(f"Errore di serializzazione JSON: {te} - Dati: {self.partita}")
                ...
                logging.getLogger("CatastoGUI").error(f"Errore durante l'esportazione JSON della partita: {e}")
...
class ModificaPartitaDialog(QDialog):
    ...
    def _save_changes(self):
        ...
        try:
            logging.getLogger("CatastoGUI").info(f"Tentativo di aggiornare i dati generali della partita ID {self.partita_id} con: {dati_modificati}")
            self.db_manager.update_partita(self.partita_id, dati_modificati)
            logging.getLogger("CatastoGUI").info(f"Dati generali della Partita ID {self.partita_id} aggiornati con successo.")
            ...
            logging.getLogger("CatastoGUI").warning(f"Violazione di unicità aggiornando partita ID {self.partita_id}: {uve.message}")
            ...
            logging.getLogger("CatastoGUI").error(f"Partita ID {self.partita_id} non trovata per aggiornamento: {nfe.message}")
            QMessageBox.critical(self, "Errore Aggiornamento", str(nfe.message))
            self.reject()
        except DBDataError as dde:
            logging.getLogger("CatastoGUI").warning(f"Errore dati aggiornando partita ID {self.partita_id}: {dde.message}")
            QMessageBox.warning(self, "Errore Dati Non Validi", f"Impossibile salvare i dati generali:\n{dde.message}")
        except DBMError as dbe:
            logging.getLogger("CatastoGUI").error(f"Errore DB aggiornando partita ID {self.partita_id}: {dbe.message}", exc_info=True)
            QMessageBox.critical(self, "Errore Database", f"Errore salvataggio dati generali:\n{dbe.message}")
        except AttributeError as ae:
            logging.getLogger("CatastoGUI").critical(f"Errore di attributo (possibile bug) aggiornando partita ID {self.partita_id}: {ae}", exc_info=True)
            QMessageBox.critical(self, "Errore Interno Applicazione", f"Errore interno (AttributeError):\n{ae}")
        except Exception as e:
            logging.getLogger("CatastoGUI").critical(f"Errore critico imprevisto aggiornando partita ID {self.partita_id}: {e}", exc_info=True)
...
# All'interno della classe ModificaPartitaDialog in prova.py
    ...
    def _load_possessori_associati(self):
        ...
        logging.getLogger("CatastoGUI").info(f"DEBUG: Inizio _load_possessori_associati per partita ID: {self.partita_id}")
        ...
        logging.getLogger("CatastoGUI").info(f"Caricamento possessori associati per partita ID: {self.partita_id}")
        try:
            ...
            logging.getLogger("CatastoGUI").info(f"DEBUG: Dati possessori ricevuti dal DB: {possessori}") # Stampa i dati ricevuti 
            
            if possessori: # Se la lista 'possessori' non è vuota
                self.possessori_table.setRowCount(len(possessori))
                logging.getLogger("CatastoGUI").info(f"DEBUG: Tabella impostata a {len(possessori)} righe.")
                for row_idx, poss_data in enumerate(possessori): # Itera sui dizionari restituiti
                    logging.getLogger("CatastoGUI").info(f"DEBUG: Popolamento riga {row_idx}, dati: {poss_data}")
                    col = 0
                ...
                self.possessori_table.resizeColumnsToContents()
                logging.getLogger("CatastoGUI").info("DEBUG: Popolamento tabella completato.")
            else:
                logging.getLogger("CatastoGUI").info(f"DEBUG: Nessun possessore (lista vuota) per la partita ID {self.partita_id}")
                logging.getLogger("CatastoGUI").info(f"Nessun possessore trovato (lista vuota) per la partita ID {self.partita_id}")
            ...
            logging.getLogger("CatastoGUI").error(f"Errore durante il popolamento della tabella possessori per partita ID {self.partita_id}: {e}", exc_info=True)
    ...
    def _aggiungi_possessore_a_partita(self):
        logging.getLogger("CatastoGUI").debug(f"Richiesta aggiunta possessore per partita ID {self.partita_id}")
        ...
        if not selected_possessore_id or not selected_possessore_nome:
            logging.getLogger("CatastoGUI").info("Nessun possessore selezionato o creato.")
            return
        # --- Fine Blocco Selezione/Creazione Possessore ---

        logging.getLogger("CatastoGUI").info(f"Possessore selezionato/creato: ID {selected_possessore_id}, Nome: {selected_possessore_nome}")
        ...
        if not dettagli_legame:
            logging.getLogger("CatastoGUI").info("Inserimento dettagli legame annullato.")
            return
        ...
        try:
            ...
            if success: # Assumendo che aggiungi_possessore_a_partita restituisca True o sollevi eccezione
                logging.getLogger("CatastoGUI").info(f"Possessore ID {selected_possessore_id} aggiunto con successo alla partita ID {self.partita_id}")
                QMessageBox.information(self, "Successo", 
            ...
            logging.getLogger("CatastoGUI").warning(f"Violazione unicità aggiungendo possessore {selected_possessore_id} a partita {self.partita_id}: {uve_rel}")
            ...
            logging.getLogger("CatastoGUI").error(f"Errore DB aggiungendo possessore {selected_possessore_id} a partita {self.partita_id}: {dbe_rel}")
            QMessageBox.critical(self, "Errore Database",
                                 f"Errore durante l'aggiunta del possessore alla partita:\n{dbe_rel.message}")
        except AttributeError as ae: 
            logging.getLogger("CatastoGUI").critical(f"Metodo 'aggiungi_possessore_a_partita' non trovato o altro AttributeError: {ae}", exc_info=True)
            ...
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto aggiungendo possessore {selected_possessore_id} a partita {self.partita_id}: {e_rel}", exc_info=True)
    ...
    def _modifica_legame_possessore(self):
        ...
        logging.getLogger("CatastoGUI").debug(f"Richiesta modifica legame per relazione ID {id_relazione_pp} (Possessore: {nome_possessore_attuale})")
        ...
        if not nuovi_dettagli_legame:
            logging.getLogger("CatastoGUI").info("Modifica dettagli legame annullata.")
            return
        ...
        try:
            ...
            if success:
                logging.getLogger("CatastoGUI").info(f"Legame ID {id_relazione_pp} aggiornato con successo.")
            ...
            logging.getLogger("CatastoGUI").error(f"Errore DB aggiornando legame {id_relazione_pp}: {dbe_legame}")
            QMessageBox.critical(self, "Errore Database",
                                 f"Errore durante l'aggiornamento del legame:\n{dbe_legame.message}")
        except AttributeError as ae:
            logging.getLogger("CatastoGUI").critical(f"Metodo 'aggiorna_legame_partita_possessore' non trovato: {ae}", exc_info=True)
            ...
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto aggiornando legame {id_relazione_pp}: {e_legame}", exc_info=True)
    ...
    def _rimuovi_possessore_da_partita(self):
        ...
        reply = QMessageBox.question(self, "Conferma Rimozione Legame",
                                     ...
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            logging.getLogger("CatastoGUI").debug(f"Richiesta rimozione legame ID {id_relazione_pp}")
            try:
                # Metodo da creare in CatastoDBManager:
                ...
                if success:
                    logging.getLogger("CatastoGUI").info(f"Legame ID {id_relazione_pp} rimosso con successo.")
                ...
                logging.getLogger("CatastoGUI").warning(f"Tentativo di rimuovere legame ID {id_relazione_pp} non trovato: {nfe_rel}")
                ...
                logging.getLogger("CatastoGUI").error(f"Errore DB rimuovendo legame {id_relazione_pp}: {dbe_rel}")
                QMessageBox.critical(self, "Errore Database",
                                     f"Errore durante la rimozione del legame:\n{dbe_rel.message}")
            except AttributeError as ae:
                logging.getLogger("CatastoGUI").critical(f"Metodo 'rimuovi_possessore_da_partita' non trovato: {ae}", exc_info=True)
                QMessageBox.critical(self, "Errore Implementazione", "Funzionalità per rimuovere legame non completamente implementata.")
            except Exception as e_rel:
                logging.getLogger("CatastoGUI").critical(f"Errore imprevisto rimuovendo legame {id_relazione_pp}: {e_rel}", exc_info=True)
                QMessageBox.critical(self, "Errore Imprevisto", f"Si è verificato un errore: {e_rel}")
    ...
    def _save_changes(self): # Questo ora salva SOLO i dati generali della partita
        ...
        try:
            logging.getLogger("CatastoGUI").info(f"Tentativo di aggiornare la partita ID {self.partita_id} con i dati: {dati_modificati}")
            self.db_manager.update_partita(self.partita_id, dati_modificati)
            
            # Se update_partita NON solleva eccezioni, l'operazione è andata a buon fine
            logging.getLogger("CatastoGUI").info(f"Partita ID {self.partita_id} aggiornata con successo nel database.")
            self.accept() # Chiude il dialogo e restituisce QDialog.Accepted

        except DBUniqueConstraintError as uve:
            logging.getLogger("CatastoGUI").warning(f"Violazione di unicità durante l'aggiornamento della partita ID {self.partita_id} con numero {dati_modificati.get('numero_partita')}: {uve}")
            QMessageBox.warning(self, "Errore di Unicità",
            ...
            logging.getLogger("CatastoGUI").error(f"Partita ID {self.partita_id} non trovata per l'aggiornamento: {nfe}")
            ...
            logging.getLogger("CatastoGUI").warning(f"Errore nei dati forniti per l'aggiornamento della partita ID {self.partita_id}: {dde}")
            ...
            logging.getLogger("CatastoGUI").error(f"Errore database gestito durante l'aggiornamento della partita ID {self.partita_id}: {dbe}")
            QMessageBox.critical(self, "Errore Database",
            ...
            logging.getLogger("CatastoGUI").critical(f"Errore di attributo (probabile bug di programmazione) durante l'aggiornamento della partita ID {self.partita_id}: {ae}", exc_info=True)
            ...
            logging.getLogger("CatastoGUI").critical(f"Errore critico e imprevisto durante il salvataggio delle modifiche alla partita ID {self.partita_id}: {e}", exc_info=True)
            QMessageBox.critical(self, "Errore Critico Imprevisto",
                                 f"Si è verificato un errore di sistema imprevisto durante il salvataggio:\n"
                                 f"{type(e).__name__}: {e}\n"
                                 "Controllare i log dell'applicazione per il traceback completo.")
            logging.getLogger("CatastoGUI").error(f"Errore salvataggio dati generali partita ID {self.partita_id}: {e}", exc_info=True)
...
class ModificaPossessoreDialog(QDialog):
    ...
    def _cambia_comune_riferimento(self):
        ...
        if dialog.exec_() == QDialog.Accepted and dialog.selected_comune_id:
            ...
            logging.getLogger("CatastoGUI").info(f"Nuovo comune di riferimento selezionato per possessore (non ancora salvato): ID {self.selected_comune_ref_id}, Nome: {dialog.selected_comune_name}")


    def _save_changes(self):
        logging.getLogger("CatastoGUI").info(f"DEBUG: _save_changes chiamato per possessore ID {self.possessore_id}") # NUOVA STAMPA
        dati_modificati = {
            "nome_completo": self.nome_completo_edit.text().strip(),
        ...
        logging.getLogger("CatastoGUI").info(f"DEBUG: Dati dalla UI: {dati_modificati}") # NUOVA STAMPA
        ...
        try:
            logging.getLogger("CatastoGUI").info(f"DEBUG: Chiamata a db_manager.update_possessore per ID {self.possessore_id}") # NUOVA STAMPA
            logging.getLogger("CatastoGUI").info(f"Tentativo di aggiornare il possessore ID {self.possessore_id} con i dati: {dati_modificati}")
            ...
            logging.getLogger("CatastoGUI").info(f"Possessore ID {self.possessore_id} aggiornato con successo.")
            logging.getLogger("CatastoGUI").info(f"DEBUG: db_manager.update_possessore completato per ID {self.possessore_id}") # NUOVA STAMPA
            self.accept() # Chiude il dialogo e restituisce QDialog.Accepted
        ...
        # Per ora, un gestore generico per errori DB e altri errori.
            ...
            logging.getLogger("CatastoGUI").error(f"Errore DB durante aggiornamento possessore ID {self.possessore_id}: {dbe_poss}", exc_info=True)
            QMessageBox.critical(self, "Errore Database",
            ...
            logging.getLogger("CatastoGUI").critical(f"Metodo 'update_possessore' non trovato o altro AttributeError: {ae}", exc_info=True)
            ...
            logging.getLogger("CatastoGUI").critical(f"Errore critico imprevisto durante il salvataggio del possessore ID {self.possessore_id}: {e_poss}", exc_info=True)
            QMessageBox.critical(self, "Errore Critico Imprevisto",
...
class PossessoriComuneDialog(QDialog):
    ...
    def apri_modifica_possessore_selezionato(self):
        logging.getLogger("CatastoGUI").debug("DEBUG: apri_modifica_possessore_selezionato chiamato.") # NUOVA STAMPA
        possessore_id = self._get_selected_possessore_id()
        if possessore_id is not None:
            logging.getLogger("CatastoGUI").debug(f"DEBUG: ID Possessore selezionato: {possessore_id}") # NUOVA STAMPA
            dialog = ModificaPossessoreDialog(self.db_manager, possessore_id, self)
            
            dialog_result = dialog.exec_() # Salva il risultato
            logging.getLogger("CatastoGUI").debug(f"DEBUG: ModificaPossessoreDialog.exec_() restituito: {dialog_result} (Accepted è {QDialog.Accepted})") # NUOVA STAMPA

            if dialog_result == QDialog.Accepted:
                logging.getLogger("CatastoGUI").info("DEBUG: ModificaPossessoreDialog accettato. Ricaricamento dati possessori...") # NUOVA STAMPA
                QMessageBox.information(self, "Modifica Possessore", 
                                        "Modifiche al possessore salvate con successo.")
                self.load_possessori_data()
            else:
                logging.getLogger("CatastoGUI").info("DEBUG: ModificaPossessoreDialog non accettato (probabilmente Annulla o errore nel salvataggio).") # NUOVA STAMPA
        else:
            logging.getLogger("CatastoGUI").warning("DEBUG: Tentativo di modificare possessore, ma nessun ID selezionato.") # NUOVA STAMPA
    ...
    def load_possessori_data(self):
        ...
        try:
            ...
            if possessori_list:
                ...
                logging.getLogger("CatastoGUI").info(f"Nessun possessore trovato per il comune ID: {self.comune_id}")
        # ... (gestione eccezioni come prima) ...
        except Exception as e:
            logging.getLogger("CatastoGUI").error(f"Errore durante il caricamento dei possessori per comune ID {self.comune_id}: {e}", exc_info=True)
            QMessageBox.critical(self, "Errore Caricamento Dati", f"Si è verificato un errore: {e}")
        finally:
            self.possessori_table.setSortingEnabled(True)
    ...
    def load_partite_data(self): # Assicurati che questo metodo esista e funzioni
        ...
        try:
            ...
            if partite_list:
                ...
                logging.getLogger("CatastoGUI").info(f"Nessuna partita trovata per il comune ID: {self.comune_id}")
        except Exception as e:
            logging.getLogger("CatastoGUI").error(f"Errore durante il caricamento delle partite per comune ID {self.comune_id}: {e}", exc_info=True)
            QMessageBox.critical(self, "Errore Caricamento Dati", f"Si è verificato un errore: {e}")
        finally:
            self.partite_table.setSortingEnabled(True)
...
class ModificaLocalitaDialog(QDialog):
    ...
    def _load_localita_data(self):
        ...
        if self.localita_data_originale.get('comune_id') != self.comune_id_parent:
            logging.getLogger("CatastoGUI").warning(f"Incoerenza Comune ID: Località {self.localita_id} appartiene a comune {self.localita_data_originale.get('comune_id')} ma aperta da contesto comune {self.comune_id_parent}")
    ...
    def _save_changes(self):
        ...
        try:
            logging.getLogger("CatastoGUI").info(f"Tentativo di aggiornare località ID {self.localita_id} con: {dati_modificati}")
            self.db_manager.update_localita(self.localita_id, dati_modificati) # Chiamata al metodo

            logging.getLogger("CatastoGUI").info(f"Località ID {self.localita_id} aggiornata con successo.")
            self.accept() # Chiudi se ha successo

        except DBUniqueConstraintError as uve:
            logging.getLogger("CatastoGUI").warning(f"Violazione unicità per località ID {self.localita_id}: {uve.message}")
            QMessageBox.warning(self, "Errore di Unicità",
                                f"Impossibile salvare le modifiche:\n{uve.message}\n"
            ...
            logging.getLogger("CatastoGUI").error(f"Località ID {self.localita_id} non trovata per l'aggiornamento: {nfe.message}")
            QMessageBox.critical(self, "Errore Aggiornamento", str(nfe.message))
            self.reject() # Chiudi perché il record non esiste più
        except DBDataError as dde:
            logging.getLogger("CatastoGUI").warning(f"Errore dati per località ID {self.localita_id}: {dde.message}")
            QMessageBox.warning(self, "Errore Dati Non Validi",
                                f"Impossibile salvare le modifiche:\n{dde.message}")
        except DBMError as dbe:
            logging.getLogger("CatastoGUI").error(f"Errore DB aggiornando località ID {self.localita_id}: {dbe.message}")
            ...
            logging.getLogger("CatastoGUI").critical(f"Metodo 'update_localita' non trovato o altro AttributeError: {ae}", exc_info=True)
            QMessageBox.critical(self, "Errore Implementazione", "Funzionalità non completamente implementata.")
        except Exception as e:
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto salvataggio località ID {self.localita_id}: {e}", exc_info=True)
...
class PeriodoStoricoDetailsDialog(QDialog):
    ...
    def _save_changes(self):
        ...
        try:
            ...
            logging.getLogger("CatastoGUI").error(f"Errore salvataggio periodo storico ID {self.periodo_id}: {str(e)}")
            QMessageBox.critical(self, "Errore Salvataggio", str(e))
        except Exception as e_gen:
            logging.getLogger("CatastoGUI").critical(f"Errore imprevisto salvataggio periodo storico ID {self.periodo_id}: {str(e_gen)}", exc_info=True)
