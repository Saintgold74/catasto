...
SET PG_DB_ADMIN=postgres  REM Database a cui connettersi per creare/droppare catasto_storico
...
REM --- Drop Database (connesso a 'postgres' o un altro DB esistente) ---
...
FOR %%F IN (%SQL_FILES%) DO (
    ...
    psql -h %PG_HOST% -p %PG_PORT% -U %PG_USER% -d %PG_DB_TARGET% -f "%SCRIPT_DIR%%%F"
    IF ERRORLEVEL 1 (
