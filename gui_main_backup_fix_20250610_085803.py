...
try:
    from fuzzy_search_widget_expanded import add_enhanced_fuzzy_search_tab_to_main_window as add_fuzzy_search_tab_to_main_window
    EXPANDED_EXPANDED_FUZZY_SEARCH_AVAILABLE = True
    print('[INIT] Ricerca fuzzy disponibile')
except:
    EXPANDED_EXPANDED_FUZZY_SEARCH_AVAILABLE = False
...
class CatastoMainWindow(QMainWindow):
    ...
    def setup_tabs(self):
        ...
        if EXPANDED_EXPANDED_FUZZY_SEARCH_AVAILABLE:
            try:
                if self.db_manager and hasattr(self.db_manager, 'pool') and self.db_manager.pool:
                    success = integrate_expanded_fuzzy_search_widget(self, self.db_manager)
                    if success:
