...
# In dialogs.py, SOSTITUISCI l'intera classe DBConfigDialog con questa
...
class DBConfigDialog(QDialog):
    def __init__(self, parent=None, initial_config: Optional[Dict] = None):
        ...
        # --- Logica di Connessione e Pre-compilazione ---
        self.local_radio.toggled.connect(self._toggle_host_field)
    ...
    def get_config_values(self, include_password: bool = False) -> Dict[str, Any]:
        ...
        config = {
            "db_type": "remote" if self.remote_radio.isChecked() else "local",
            "host": self.host_edit.text().strip() if self.remote_radio.isChecked() else "localhost",
    ...
    def _save_settings(self, config: Dict):
        """Salva le impostazioni usando QSettings."""
        settings = QSettings()
        settings.setValue("Database/Type", config["db_type"])
        ...
        if config.get("save_password") and config.get("password"):
            if keyring:
                try:
                    ...
                    self.logger.error(f"Impossibile salvare password nel keyring: {e}")
        else:
            # Se non si vuole salvare, rimuovi la vecchia chiave se esiste
            if keyring:
                try:
                    keyring.delete_password(f"meridiana_db_{config['host']}", config['user'])
                except keyring.errors.NoKeyringError:
                    pass # Non fa nulla se non c'è un backend keyring
                except Exception:
                    pass # Ignora altri errori in fase di cancellazione
        settings.sync()
    ...
    def _handle_save_and_connect(self):
        ...
        if not all([config["dbname"], config["user"], config["password"]]):
            QMessageBox.warning(self, "Dati Mancanti", "Nome DB, Utente e Password sono obbligatori.")
            return
        ...
        if test_db_manager.initialize_main_pool():
            ...
            self.accept() # Chiude il dialogo con esito positivo
        else:
            QMessageBox.critical(self, "Connessione Fallita", "Impossibile connettersi. Controlla i dati e riprova.")
